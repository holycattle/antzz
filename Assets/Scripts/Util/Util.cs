...
public class Util {
	...
	static public GameObject Find(GameObject parent, string name, bool includeDisabled) {
		...
		for (int i=0; i<childCount; i++) {
			...
			if (!t || !t.gameObject)
				continue;
			if (includeDisabled == false && t.gameObject.activeSelf == false)
				continue;
	...
	static public GameObject[] FindObjectsWithSubString(GameObject parent, string pattern) {
		...
		foreach (Transform child in parent.transform) {
			...
			if (child.gameObject.name.IndexOf(pattern) == -1)
				continue;
	...
	static public Bounds GetBounds2D(GameObject go) {
		...
		foreach (Transform t in go.transform) {
			...
			if (currBounds.min.x < min.x)
				min.x = currBounds.min.x;
			if (currBounds.min.y < min.y)
				min.y = currBounds.min.y;
			
			if (currBounds.max.x > max.x)
				max.x = currBounds.max.x;
			if (currBounds.max.y > max.y)
				max.y = currBounds.max.y;
